...
export const CartProvider = ({ children }) => {
  const [cartMeta, dispatchCartMeta] = useReducer(cartMetaReducer);
  ...
  const handleCheckout = () => {
    if (!state.hosted_checkout_url) return;

    setIsLoading(true);
    window.location = state.hosted_checkout_url;
  };

  return (
    <CartMetaContext.Provider value={{ ...cartMeta, openCart, closeCart }}>
      <CartDispatchContext.Provider value={{ setCart, addToCart, handleCheckout }}>
